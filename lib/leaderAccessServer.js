...
import { normalizeName, normalizeCtCode, normalizeUnit, isValidUnit, isValidCtCode } from './leaderAccess';
...
export function parseAccessList(buf) {
  ...
  for (const raw of lines) {
    ...
    records.push({ name, unit: normalizeUnit(u), ctCode });
  }
...
export function findLeaderMatch(records, input) {
  ...
  // Re-assert the closed sets server-side before comparing (never trust input).
  if (!name || !isValidUnit(unit) || !isValidCtCode(ctCode)) return false;
  return (records || []).some(
